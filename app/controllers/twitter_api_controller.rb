...
class TwitterApiController < ApplicationController
  ...
  def fetch_tweets
    ...
    TweetStream::Client.new.locations(-125.0011, 24.9493, -66.9326, 49.5904) do |tweet|
      begin
        @new_tweet = Tweet.new
        ...
        # Getting to the centre of the location bounded box.
        @location_count = tweet.place.bounding_box.coordinates[0].length - 1
        @location_sum = 0
        for i in 0..(@location_count-1)
          @location_sum += tweet.place.bounding_box.coordinates[0][i][1]
        end
        @new_tweet.latitude = @location_sum/@location_count
        @location_sum = 0
        for i in 0..(@location_count-1)
          @location_sum += tweet.place.bounding_box.coordinates[0][i][0]
        end
        @new_tweet.longitude = @location_sum/@location_count
